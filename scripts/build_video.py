...
AI_DIR = BASE_DIR / "videos" / "ai"
SALES_DIR = BASE_DIR / "videos" / "sales"
...
def choose_video_type() -> str:
    return random.choices(["sales", "ai"], weights=[90, 10], k=1)[0]
...
def run_ffmpeg(meme_path: Path, main_video_path: Path, output_path: Path) -> None:
    ...
    filter_complex = (
        ...
        f"[0:a:0]aresample={TARGET_AUDIO_RATE},"
        f"aformat=sample_rates={TARGET_AUDIO_RATE}:channel_layouts=stereo[a0];"
        f"[1:a:0]aresample={TARGET_AUDIO_RATE},"
        f"aformat=sample_rates={TARGET_AUDIO_RATE}:channel_layouts=stereo[a1];"
